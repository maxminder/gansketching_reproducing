...
def apply_shift(id, g, mean_latent, latents, s, layers, w_plus=False, trunc=0.5):
...
if (__name__ == '__main__'):
    ...
    with jt.no_grad():
        ...
        for i in range(num):
            print(i*slice)
            ims = apply_shift(k, netG, mean_latent, latents, -i*slice, layers, trunc=args.truncation)
            save_ims(f'./{args.save_dir}/neg_'+str(num-1-i), ims)
            ims = apply_shift(k, netG, mean_latent, latents, i*slice, layers, trunc=args.truncation)
