...
class DiffAugment(jt.nn.Module):
    ...
    def forward(self, x):
        if self.policy:
            if not self.channels_first:
                # x = x.permute(0, 3, 1, 2)
                x = jt.permute(x,(0,3,1,2))
            for p in self.policy.split(','):
                for f in AUGMENT_FNS[p]:
                    x = f(x)
            if not self.channels_first:
                # x = x.permute(0, 2, 3, 1) 存疑
                x = jt.permute(x,(0,2,3,1))
            # x = x.contiguous()
        return x


def rand_brightness(x):
    # x = x + (jt.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device) - 0.5) 存疑
...
def rand_cutout(x, ratio=0.5):
    ...
    grid_batch, grid_x, grid_y = jt.meshgrid(
        jt.arange(x.size(0), dtype=jt.int64, device=x.device),
        jt.arange(cutout_size[0], dtype=jt.int64, device=x.device),
        jt.arange(cutout_size[1], dtype=jt.int64, device=x.device),
    )
