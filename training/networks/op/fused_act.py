...
def fused_leaky_relu(input, bias=None, negative_slope=0.2, scale=2 ** 0.5):
    if bias is not None:
        ...
        elif  input.ndim - bias.ndim - 1 == 1:
            return jt.nn.leaky_relu(input + bias.view(1, bias.shape[0], 1), scale=0.2) * scale
        else:
            return jt.nn.leaky_relu(input + bias.view(1, bias.shape[0], 1, 1), scale=0.2) * scale
